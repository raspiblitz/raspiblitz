...
if [ "$1" == "mainnet" ]; then
  ...
  lndcli_alias="/usr/local/bin/lncli -n=mainnet --rpcserver=localhost:12009 --macaroonpath=${lndHomeDir}/data/chain/bitcoin/testnet/readonly.macaroon --tlscertpath=${lndHomeDir}/tls.cert"
  netprefix="t"
elif [ "$1" == "signet" ]; then
