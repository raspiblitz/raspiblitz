...
EOF
  ...
  if [ "${runBehindTor}" = "on" ]; then
    ...
    /home/admin/config.scripts/tor.onion-service.sh cryptoadvance-specter 443 25441
  fi
...
if [ "$1" = "0" ] || [ "$1" = "off" ]; then
  ...
  if [ "${runBehindTor}" = "on" ]; then
    /home/admin/config.scripts/tor.onion-service.sh off cryptoadvance-specter
  fi
