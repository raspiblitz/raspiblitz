...
echo "# Running: bitcoin-to-knots.sh $*"
...
downloadDir="/home/$(whoami)/download/bitcoin-to-knots"
...
if [ "${mode}" = "tested" ]; then
  ...
  echo "# bitcoin-to-knots.sh tested"
  ...
  echo "# bitcoin-to-knots.sh reckless"
  ...
  if curl --output /dev/null --silent --head --fail \
    https://bitcoinknots.org/files/29.x/${pathVersion}/SHA256SUMS.asc; then
    echo "# OK version exists at https://bitcoinknots.org/files/29.x/${pathVersion}"
...
if [ "${mode}" = "tested" ] || [ "${mode}" = "reckless" ] || [ "${mode}" = "custom" ]; then
  ...
  curl -s "https://api.github.com/repos/bitcoinknots/guix.sigs/contents/builder-key" |
    jq -r '.[].download_url' | while read url; do curl -s "$url" | gpg --import; done

  # download signed binary sha256 hash sum file
  wget --prefer-family=ipv4 --progress=bar:force -O SHA256SUMS https://bitcoinknots.org/files/29.x/${bitcoinVersion}/SHA256SUMS
  # download the signed binary sha256 hash sum file and check
  wget --prefer-family=ipv4 --progress=bar:force -O SHA256SUMS.asc https://bitcoinknots.org/files/29.x/${bitcoinVersion}/SHA256SUMS.asc
  ...
  if [ "${mode}" = "custom" ] && [ "$3" = "skipverify" ]; then
    ...
    if gpg --verify SHA256SUMS.asc; then
      echo
      echo "****************************************"
      echo "OK --> BITCOIN KNOTS MANIFEST IS CORRECT"
      echo "****************************************"
      echo
    else
      echo
      ...
      echo "# /home/admin/config.scripts/bitcoin-to-knots.update.sh custom ${bitcoinVersion:-<version>} skipverify"
      exit 1
    fi
  fi
  ...
  wget https://bitcoinknots.org/files/29.x/${pathVersion}/${binaryName}
  ...
  if ! sha256sum -c --ignore-missing SHA256SUMS; then
    ...
    echo "# FAIL # Downloaded BITCOIN KNOTS BINARY CHECKSUM:"
...
if [ "${mode}" = "tested" ] || [ "${mode}" = "reckless" ] || [ "${mode}" = "custom" ]; then
  ...
  sudo 	ctl stop sbitcoind 2>/dev/null
  echo
