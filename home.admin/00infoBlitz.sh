...
if [ "$wallet_unlocked" -gt 0 ] ; then
  ...
  if [ ${ln_tor} -eq 1 ]; then
    LNDselfTest=$(proxychains4 nmap -Pn -p 9735 -sT -v "$(sudo -u bitcoin /usr/local/bin/lncli getinfo|grep "uris" -A1|tr -d '\n'|cut -d'"' -f4|cut -d@ -f2|cut -d: -f1)" 2>/dev/null|grep -c "9735/tcp open")
